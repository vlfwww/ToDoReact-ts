...
function App() {
  ...
  const [editId, setEditId] = useState(null); // Для отслеживания редактируемой задачи
  ...
  function add() {
    ...
    if (editId) {
      // Если редактируем задачу, обновляем её
      setArr(arr.map(el => el.id === editId ? { ...el, todo: inp } : el));
  ...
  function startEdit(id: number | SetStateAction<null>) {
    const taskToEdit = arr.find(el => el.id === id);
    if (taskToEdit) {
      setInp(taskToEdit.todo);
      setEditId(id); 
    }
  }
      ...
      <div>
        ...
        {arr.map((el) => (
          <div key={el.id}>
            ...
            <button onClick={() => startEdit(el.id)}>Edit</button>
          </div>
        ))}
      </div>
    </>
